...
const TimelineItem = ({ item, isLast }) => {
  ...
  const itemColor = colors[item.type] || colors.info;
  const colorBase = itemColor.split(' ')[0];
  const colorBorder = itemColor.split(' ')[1];
  ...
  return (
    <div className="relative pl-8 pb-8 last:pb-0">
      ...
      <div className={`absolute left-0 top-1 w-6 h-6 rounded-full border-2 bg-kairos-bg flex items-center justify-center z-10 ${colorBase} ${colorBorder}`}>
        <Icon size={12} />
      </div>
